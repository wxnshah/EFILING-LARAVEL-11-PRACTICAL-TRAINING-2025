                                ...
                                <form class="needs-validation" novalidate action="{{ route('PostAddInformation') }}" enctype="multipart/form-data" method="POST" onSubmit="return capture();">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <select class="form-control select2" name="id_gender">
                                                ...
                                                @foreach ($query_gender as $data_gender)
                                                    <option value="{{ $data_gender->id_gender }}" {{ $data_gender->id_gender == old('id_gender') ? "selected" : "" }}>{{ $data_gender->name_gender }}</option>
                                                @endforeach
                                            </select>
                                            @error('id_gender')
                                    ...
                                    <div class="form-group row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" placeholder="Sila Masukkan No Matrik" name="no_matrik" value="{{ old('no_matrik') }}">
                                            @error('no_matrik')
                                                <font color="red">{{ $message }}</font>
                                            @enderror
                                        </div>
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control datepicker" placeholder="Sila Masukkan Tarikh Lapor Diri" name="tarikh_lapor" value="{{ old('tarikh_lapor') }}">
                                            @error('tarikh_lapor')
                                                <font color="red">{{ $message }}</font>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <select class="form-control select2" name="id_course">
                                                ...
                                                @foreach ($query_course as $data_course)
                                                    <option value="{{ $data_course->id_course }}" {{ $data_course->id_course == old('id_course') ? "selected" : "" }}>{{ $data_course->name_course }}</option>
                                                @endforeach
                                            </select>
                                            @error('id_course')
                                                <font color="red">{{ $message }}</font>
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <select class="form-control select2" name="id_ipta">
                                                ...
                                                @foreach ($query_ipta as $data_ipta)
                                                    <option value="{{ $data_ipta->id_ipta }}" {{ $data_ipta->id_ipta == old('id_ipta') ? "selected" : "" }}>{{ $data_ipta->name_ipta }}</option>
                                                @endforeach
                                            </select>
                                            @error('id_ipta')
                                                <font color="red">{{ $message }}</font>
